/**
 * Device, browser, and OS detection utilities
 */
 ...
 */
export function detectiPhoneModel(screenWidth: number, screenHeight: number, pixelRatio: number): string {
 ...
 */
export function detectiPadModel(screenWidth: number, screenHeight: number, pixelRatio: number): string {
  const width = Math.min(screenWidth, screenHeight)
  const height = Math.max(screenWidth, screenHeight)
 ...
 */
export function parseDevice(ua: string, screenWidth?: number, screenHeight?: number, pixelRatio?: number): string {
